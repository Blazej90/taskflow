...
export class TasksService {
  ...
  // Loading states - rozdzielone
  private hasLoaded = signal(false);
  readonly bootLoading = computed(() => !this.hasLoaded());

  // loading dla operacji (nie blokuje przy braku danych)
  readonly loading = computed(() => {
  ...
  constructor(
    ...
    this.repo.tasks$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (tasks) => {
        ...
        this.hasLoaded.set(true); // ZAWSZE ustaw na true po pierwszym ładowaniu
      },
      error: (err) => {
        console.error('Failed to load tasks:', err);
        this.hasLoaded.set(true); // Nawet przy błędzie - przestań pokazywać loading
        this._tasks.set([]);
      },
    });
